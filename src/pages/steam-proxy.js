...
app.get('/api/sync-steam-apps', async (req, res) => {
    try {
        ...
        fs.writeFileSync(path.join(__dirname, 'steam-apps.json'), JSON.stringify(data.applist.apps, null, 2));
        res.json({ message: 'Daftar game Steam berhasil di-sync!', total: data.applist.apps.length });
    } catch (err) {
...
app.get('/api/search', async (req, res) => {
    ...
    let allApps = [];
    try {
        allApps = JSON.parse(fs.readFileSync(path.join(__dirname, 'steam-apps.json')));
    } catch (err) {
        return res.status(500).json({ error: 'Daftar game lokal belum tersedia, silakan sync dulu.' });
    }
    // Cari game yang namanya mengandung query (case-insensitive)
    const matches = allApps.filter(app => app.name.toLowerCase().includes(query.toLowerCase()));
